...
<style>
    ...
    /* Sepete eklendiğinde kartın hafif parlama/yükselme tepkisi */
...
<div class="min-h-screen bg-gradient-to-b from-[#EAF4FF] via-[#DCEEFF] to-[#EAF4FF] al-font-body text-[#0B2545] selection:bg-[#FFB347] selection:text-[#0A1830]"
    ...
    <nav
        ...
        <div class="hidden md:flex items-center gap-9 al-font-mono text-xs tracking-widest text-[#0B2545]/60">
            ...
            <a href="<?php echo e(route('magaza')); ?>" class="text-[#0B2545] opacity-100">MAĞAZA</a>
        </div>
    ...
    <section class="max-w-7xl mx-auto px-6 pb-32">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?>[if BLOCK]><![endif]<?php endif; ?><?php $__currentLoopData = $urunler; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $urun): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div x-show="active === 'all' || active === '<?php echo e($urun['kategori']); ?>'"
                x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95"
                x-transition:enter-end="opacity-100 scale-100"
                :class="addedItems['<?php echo e($urun['kod']); ?>'] ? 'm-card-added' : ''"
                ...
                style="border-left: 3px solid <?php echo e($urun['renk']); ?>;">
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?>[if BLOCK]><![endif]<?php endif; ?><?php if($urun['eskiFiyat']): ?>
                <div
                    ...
                    %<?php echo e(round((1 - $urun['fiyat'] / $urun['eskiFiyat']) * 100)); ?> İNDİRİM
                </div>
                ...
                <div class="p-7 flex flex-col h-full">
                    <div class="flex items-center justify-between mb-5">
                        <div class="m-badge w-11 h-11 rounded-xl flex items-center justify-center al-font-mono text-[10px] font-bold text-white shrink-0"
                            style="background-color: <?php echo e($urun['renk']); ?>;">
                            <?php echo e($urun['kod']); ?>

                        </div>
                        <span class="al-font-mono text-[10px] tracking-widest font-bold"
                            style="color: <?php echo e($urun['renk']); ?>;">
                            <?php echo e($urun['etiket']); ?>
                    ...
                    <p class="text-sm text-[#0B2545]/60 leading-relaxed mb-6 flex-grow">
                        <?php echo e($urun['aciklama']); ?>

                    </p>

                    <div
                        class="al-font-mono text-[11px] text-[#0B2545]/50 space-y-1.5 mb-6 pt-5 border-t border-[#0B2545]/10">
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?>[if BLOCK]><![endif]<?php endif; ?><?php $__currentLoopData = $urun['specs']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $spec): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="flex items-center gap-2">
                            <span class="w-1 h-1 rounded-full shrink-0"
                                style="background-color: <?php echo e($urun['renk']); ?>;"></span>
                            <?php echo e($spec); ?>
                    ...
                    <div class="flex items-end justify-between mb-5">
                        <div>
                            ...
                            <p class="al-font-mono text-xs text-[#0B2545]/40 line-through mb-0.5">
                                <?php echo e(number_format($urun['eskiFiyat'], 0, ',', '.')); ?> ₺
                            </p>
                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?>[if ENDBLOCK]><![endif]<?php endif; ?>
                            <p class="al-font-display text-2xl font-extrabold text-[#0B2545]">
                                <?php echo e(number_format($urun['fiyat'], 0, ',', '.')); ?> ₺
                            </p>
                        </div>
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?>[if BLOCK]><![endif]<?php endif; ?><?php if($urun['stok']): ?>
                        <span
                    ...
                    <button type="button" <?php if($urun['stok']): ?> @click="
                            Livewire.dispatch('addToCart', { kod: <?php echo \Illuminate\Support\Js::from($urun['kod'])->toHtml() ?>, ad: <?php echo \Illuminate\Support\Js::from($urun['ad'])->toHtml() ?>, fiyat: <?php echo e($urun['fiyat']); ?>, renk: <?php echo \Illuminate\Support\Js::from($urun['renk'])->toHtml() ?> });
                            addItem('<?php echo e($urun['kod']); ?>');
                        " <?php endif; ?> :disabled="!<?php echo e($urun['stok'] ? 'true' : 'false'); ?>"
                        class="relative w-full h-11 al-font-mono text-xs font-bold tracking-widest rounded-xl overflow-hidden transition-all duration-300 <?php echo e($urun['stok'] ? 'bg-[#0B2545] text-white hover:bg-[#2F6FED]' : 'bg-[#0B2545]/10 text-[#0B2545]/40 cursor-not-allowed'); ?>">
                        <span class="m-btn-label absolute inset-0 flex items-center justify-center gap-2"
                            x-show="!addedItems['<?php echo e($urun['kod']); ?>']"
                            x-transition:enter="transition ease-out duration-200"
                            x-transition:enter-start="opacity-0 -translate-y-1"
                            x-transition:enter-end="opacity-100 translate-y-0"><?php echo e($urun['stok'] ? 'SEPETE EKLE' : 'STOK
                            YOK'); ?></span>

                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?>[if BLOCK]><![endif]<?php endif; ?><?php if($urun['stok']): ?>
                        <span
                            class="m-btn-check absolute inset-0 flex items-center justify-center gap-2 bg-emerald-500 text-white"
                            x-show="addedItems['<?php echo e($urun['kod']); ?>']" x-cloak
                            x-transition:enter="transition ease-out duration-200"
                            x-transition:enter-start="opacity-0 translate-y-1"
     ...
     Gerçek sepet: session tabanlı, checkout (odeme) akışına bağlı Livewire bileşeni 
    <?php
...
$__key ??= \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::generateKey('lw-3838125233-0', $__key);
...
<script>
    // Navbar sepet rakamı için global Alpine store: sayfa yüklenirken gerçek session sepetinden başlar.
    document.addEventListener('alpine:init', () => {
        Alpine.store('cart', { count: <?php echo e($sepetAdet); ?> });
    });
...
<?php echo $__env->make('layouts.full', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\apoca\Desktop\livewire-projem\resources\views/magaza.blade.php ENDPATH**/ ?>