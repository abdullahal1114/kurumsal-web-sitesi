...
<div class="al-font-body min-h-screen w-full relative overflow-hidden bg-gradient-to-br from-[#0B2545] via-[#12315F] to-[#0B2545] flex items-center justify-center p-6">
    ...
    <style>
        ...
        .login-pulse { animation: loginPulse 4.5s ease-in-out infinite; }
        .login-pulse-delay { animation: loginPulse 4.5s ease-in-out infinite; animation-delay: 1.6s; }

        @keyframes loginFadeUp {
            from { opacity: 0; transform: translateY(24px); }
            to { opacity: 1; transform: translateY(0); }
        }
        .login-fade { opacity: 0; animation: loginFadeUp 0.7s cubic-bezier(0.16,1,0.3,1) forwards; }
        .login-fade-delay { animation-delay: 0.15s; }

        @keyframes alLiveDot {
            0%, 100% { box-shadow: 0 0 0 0 rgba(52, 211, 153, 0.55); }
            50% { box-shadow: 0 0 0 5px rgba(52, 211, 153, 0); }
        }
        .al-live-dot { animation: alLiveDot 2.2s ease-in-out infinite; }

        .al-social-icon { transition: all 0.3s ease; }
        ...
        .al-input-wrap:focus-within .al-input-icon { color: #2F6FED; }
        .al-input-wrap:focus-within { border-color: #2F6FED; }
    </style>

     Grid deseni + ışık çizgileri 
    ...
    <div class="absolute -bottom-40 left-1/4 w-[500px] h-[500px] bg-[#FF9F45]/10 rounded-full blur-3xl"></div>
     ...
     Logo 
    <a href="<?php echo e(route('home')); ?>" class="absolute top-8 left-8 al-font-display text-2xl font-bold tracking-tight text-white z-30 hover:opacity-80 transition-opacity duration-300">
        AL<span class="text-[#FF9F45]">.</span>TECHNOLOGY
    </a>

     Sistem durumu rozeti 
    <div class="absolute top-8 right-8 z-30 hidden sm:flex items-center gap-2 bg-white/5 border border-white/10 backdrop-blur-md rounded-full pl-3 pr-4 py-1.5">
        ...
        <a href="https://twitter.com/altechnology58" target="_blank"
           class="al-social-icon w-11 h-11 rounded-full border border-white/15 bg-white/5 backdrop-blur-md flex items-center justify-center text-white/70 hover:text-[#FF9F45]">
            <svg class="w-[16px] h-[16px]" viewBox="0 0 24 24" fill="currentColor">
                <path d="M18.9 2.6h3.4l-7.5 8.5 8.8 11.6h-6.9l-5.4-7.1-6.2 7.1H1.6l8-9.1L1.2 2.6h7l4.9 6.5 5.8-6.5Zm-1.2 18.1h1.9L7.4 4.6H5.4l12.3 16.1Z"/>
            </svg>
        </a>
    </div>
    ...
    <div class="relative max-w-6xl w-full flex items-center justify-between gap-16 z-20">
        ...
        <div class="w-full max-w-md login-fade login-fade-delay" x-data="{ showPassword: false }">
            <div class="relative bg-white/95 backdrop-blur-md p-10 rounded-3xl shadow-2xl shadow-black/30 text-[#0B2545] border border-white/50 overflow-hidden">
                ...
                <form wire:submit="login" class="space-y-5">
                    ...
                    <div class="al-input-wrap flex items-center gap-3 border-2 border-[#0B2545]/10 rounded-xl px-4 py-3 transition-colors duration-300">
                    ...
                    <div class="al-input-wrap flex items-center gap-3 border-2 border-[#0B2545]/10 rounded-xl px-4 py-3 transition-colors duration-300">
                        ...
                        <input wire:model="form.password" :type="showPassword ? 'text' : 'password'" required placeholder="Şifreniz"
                               class="al-font-body w-full border-0 focus:ring-0 p-0 text-base bg-transparent placeholder:text-[#0B2545]/40">
                        <button type="button" @click="showPassword = !showPassword" class="shrink-0 text-[#0B2545]/30 hover:text-[#2F6FED] transition-colors duration-300">
                    ...
                    <div class="flex items-center justify-between text-sm pt-1">
                        <label class="flex items-center gap-2 text-[#0B2545]/60 cursor-pointer select-none">
                            <input wire:model="form.remember" type="checkbox" class="rounded border-[#0B2545]/20 text-[#2F6FED] focus:ring-[#2F6FED]/40">
                            Beni hatırla
                        </label>
                        <a href="<?php echo e(route('password.request')); ?>" wire:navigate class="text-[#2F6FED] hover:text-[#0B2545] font-medium transition-colors duration-300">
                            Şifremi unuttum
                        </a>
                    </div>
             ...
             Mobilde sosyal ikonlar (kart altında) 
...
</div><?php /**PATH C:\Users\apoca\Desktop\livewire-projem\resources\views\livewire/pages/auth/login.blade.php ENDPATH**/ ?>