...
new #[Layout('layouts.full-livewire')] class extends Component
{
    public array $items = [];
    ...
    public function mount(): void
    {
        ...
        if (empty($this->items)) {
            $this->redirectRoute('magaza', navigate: true);
        }
    }
    ...
    public function tamamla(): void
    {
        ...
        $this->orderNumber = 'AL-'.str_pad((string) $order->id, 6, '0', STR_PAD_LEFT);
        $this->completed = true;
    }
}; ?>
                    ...
                    <div class="bg-white/70 backdrop-blur-sm rounded-2xl border border-[#0B2545]/10 p-5 sticky top-8">
                        ...
                        <div class="space-y-3 mb-5">
                            ...
                            <div class="flex items-center gap-3">
                                <div class="w-9 h-9 rounded-lg flex items-center justify-center text-[10px] font-bold text-white shrink-0" style="background-color: {{ $item['renk'] }}; font-family:'JetBrains Mono',monospace;">
                                    {{ $item['kod'] }}
                                </div>
                                <div class="flex-1 min-w-0">
                                    <p class="text-xs font-bold text-[#0B2545] truncate">{{ $item['ad'] }}</p>
                                    <p class="text-[11px] text-[#0B2545]/45" style="font-family:'JetBrains Mono',monospace;">{{ $item['adet'] }} adet × {{ number_format($item['fiyat'], 0, ',', '.') }} ₺</p>
                                </div>
                                <p class="text-xs font-bold text-[#0B2545] shrink-0" style="font-family:'JetBrains Mono',monospace;">{{ number_format($item['fiyat'] * $item['adet'], 0, ',', '.') }} ₺</p>
                            </div>
                            @endforeach
                        </div>
